...
function getPoint(number, url = __dirname + "/1.png") {
  ...
  ctx.drawImage(img, 0, 0, width, height); // 将图片加载到canvas里面 // 裁剪图片
  // ctx.drawImage(img, 0, height / 7, width, height * 3 / 7); // 将图片加载到canvas里面 // 裁剪图片
  var imageData = ctx.getImageData(0, height / 7, width, height * 3 / 7);
  ...
  //   let { red: red3, blue: blue3, green: green3 } = get3(
  //     width - 200,
  //     height - 200
  //   );
  ...
  //   // console.log(redMax)
  //   for (
  //     let j = Math.round(600 / 1920 * height);
  //     j < 1500 / 1920 * height;
  //     j += 1
  //   ) {
  ...
  //   let hhMax = 0;
  //   for (
  //     let line = Math.round(800 / 1920 * height);
  //     line < 1500 / 1920 * height;
  //     line++
  //   ) {
  ...
  //     if (
  //       boxMax > hhMax * 1.5 ||
  //       (count < boxMax && boxMax > hhMax * 1.18) ||
  //       count245 > 30
  //     ) {
  //       // 得到棋盘坐标后退出
...
function auto(params) {
  ...
  // jump(box, hh);
  // console.log("休息一会儿~");
  // setTimeout(auto, 800); // 时间可以自己调整，避免新的棋盘没有落下来，就开始截屏了，这样找到的点就不准了，具体时间要看手机的性能
  // number++;
}
shell.rm("./log/*.png");
